plot_world_cum <- function(shock_type, ds_map_cum, scen, quant, adapt, land_plotted_var=NA){
  ...
  color <- c("lightyellow", "khaki1", "gold", "orange", "orangered2", "red3", "purple4")
  
  if(shock_type == "land"){
    plot <- ggplot(ds_toplot) +
      geom_sf(aes(fill = -{{land_plotted_var}})) +
      scale_fill_gradientn(colours = color, 
                           values = rescale(c(0, 0.8, 1.8, 3.5, 7.5, 15, 100)), 
                           limits = c(0,15), na.value = "lightgrey") +
      theme_light() +
      labs(fill = "Cum shock (%)",
           title = title_plot,
    ...
    plot <- ggplot(ds_toplot) +
      ...
      scale_fill_gradientn(colours = color, 
                           values = rescale(c(0, 2, 5, 10, 20, 40, 100)), 
