...
land_interpolated <- land_interpolated |> left_join(land_sel, join_by(locationid)) |> 
  mutate(land_remaining = total_land[time == 2015][1] - cumsum(land_loss_yearly))
...
land_shock_ISO_y <- land_interpolated  |> 
  ...
  mutate(land_shock_y = (land_remaining / lag(land_remaining) - 1) * 100) |> ungroup()
...
land_shock_5y_iso <- land_interpolated |> 
  ...
  mutate(land_shock_5y = (land_remaining / lag(land_remaining, 5) - 1) * 100) |> ungroup() |> 
...
land_shock_cum_iso <- land_interpolated |> 
  arrange(across(all_of(groupby_vars)), time) |> group_by(across(all_of(groupby_vars))) |>
  mutate(land_shock_cum = (land_remaining / land_remaining[time == 2020][1] - 1) * 100) |>
...
LAND_ICES_MH <- land_interpolated_ICES_MH |>
  ...
  summarise(across(c(land_loss_yearly, total_land), ~ sum(.x, na.rm = TRUE)),
            .groups = "drop") |> 
  mutate(land_remaining = total_land[time == 2015][1] - cumsum(land_loss_yearly))
...
land <- left_join(land_shock_ISO_y, world, join_by(locationid)) 
land_cum <- left_join(land_shock_cum_iso, world, join_by(locationid))

land_missing <- anti_join(world, land_shock_ISO_y, join_by(locationid))

structure <- colnames(land |> select(rcp:time))
cols_values <- tail(names(land_shock_ISO_y), 4)
...
land_cum_world_revisited <- land_cum_world |> 
...
plot_world_cum("land", land_cum_world_revisited, scen = 370, quant = 0.95, adapt = "Constant dike height")
